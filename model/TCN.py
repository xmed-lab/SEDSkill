...
from .relative import TransformerDecoderLayer
import copy
import numpy as np
from torch.hub import load_state_dict_from_url
# from bgm_block.models import fullBGM,LocalBarrierPooling

# from prototype import sample_sss

# Modified from https://github.com/yabufarha/ms-tcn/edit/master/model.py
class SingleStageTCN(nn.Module):
    def __init__(self, num_layers, input_dim, middle_dim, output_dim, dropout=0.5):
        ...
        self.head = nn.Linear(middle_dim, 128)
        # self.classfier = nn.Linear(middle_dim, output_dim)
        self.regress = nn.Linear(middle_dim, 1)
    def forward(self, x):  # Input: (N, F, T)   Output: (N, F, T)
        ...
        score = self.regress(feat)
        return score

class SingleStageTCN_Con_Loc(nn.Module):
    def __init__(self, num_layers, input_dim, middle_dim, output_dim, dropout=0.5):
        super(SingleStageTCN_Con_Loc, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
        self.conv_in = nn.Conv1d(input_dim, middle_dim, 1)
        self.layers = nn.ModuleList(
            [copy.deepcopy(DilatedResidualLayer(2 ** i, middle_dim, middle_dim)) for i in range(num_layers)])
        ...
        self.pos = LearnedPositionalEncoding(
                    1000, middle_dim+2048
                )
      
        self.relativer = TransformerDecoderLayer(64+2048, 8,64,
                                            0.1, 'relu',normalize_before=True)
        self.rel_regress = nn.Linear(middle_dim+2048, 1)
        self.rel_classifier = nn.Linear(middle_dim+2048, output_dim)
    def forwad_tcn(self, x):
        x = self.dropout(x)
        out = self.conv_in(x)
        for layer in self.layers:
            out = layer(out)
    ...
    def forward(self, x, x_exampler, local_x =None, local_x_exampler=None):  # Input: (N, F, T)   Output: (N, F, T)
        score, out = self.forwad_tcn(x)
        score_exampler, out_exampler = self.forwad_tcn(x_exampler)
       
        out = torch.cat([local_x, out],dim=1)
        
        out_exampler = torch.cat([local_x_exampler, out_exampler],dim=1)
        pos_out = self.pos(out)
        pos_out_exampler = self.pos(out_exampler)
        
        pos_out = pos_out.permute(2,0,1)
       
        new_out,_ = self.relativer(pos_out, pos_out_exampler)


       
        new_out = new_out.permute(1,2,0)
       
        feat = new_out.mean(2)
       
        relative_score = self.rel_regress(feat)
       
        return score ,score_exampler, relative_score
...
class SingleStageTCN_Con(nn.Module):
    def __init__(self, num_layers, input_dim, middle_dim, output_dim, dropout=0.5):
        super(SingleStageTCN_Con, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
        self.conv_in = nn.Conv1d(input_dim, middle_dim, 1)
        self.layers = nn.ModuleList(
            [copy.deepcopy(DilatedResidualLayer(2 ** i, middle_dim, middle_dim)) for i in range(num_layers)])
        ...
        self.relativer = TransformerDecoderLayer(64, 8,64,
                                            0.1, 'relu',normalize_before=True)
        self.rel_regress = nn.Linear(middle_dim, 1)
    def forwad_tcn(self, x):
        x = self.dropout(x)
        out = self.conv_in(x)
        for layer in self.layers:
    ...
    def forward(self, x, x_exampler, local_x =None, local_x_exampler=None):  # Input: (N, F, T)   Output: (N, F, T)
        ...
        new_out,_ = self.relativer(out, out_exampler)
